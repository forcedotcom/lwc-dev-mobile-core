...
export class AndroidDeviceManager {
    ...
    private parseEnumerationRawString(rawString: string): AndroidDevice[] {
        ...
        for (const avd of avds) {
            ...
            const avdINI = avdPath.replace(/\.avd$/, '.ini');
            ...
            if (fs.existsSync(avdINI) && fs.existsSync(configINI)) {
                const avdINIContent = CaseInsensitiveStringMap.fromString(fs.readFileSync(avdINI, 'utf8'));
                const configINIContent = CaseInsensitiveStringMap.fromString(fs.readFileSync(configINI, 'utf8'));
                ...
                const id = configINIContent.get('AvdId') ?? avd.get('Name');
                ...
                const targetType = configINIContent.get('tag.id')?.toLowerCase();

                const isPlayStore = (configINIContent.get('PlayStore.enabled')?.toLowerCase() ?? 'false') === 'true';
                ...
                const targetAPI = (
                    configINIContent.get('image.sysdir.1')?.replace(/.*(android-\d+).*/i, '$1') ??
                    avdINIContent.get('target')
                )?.replace(/android-/i, '');
                ...
                if (id && name && targetType && targetAPI) {
                    ...
                    results.push(
                        new AndroidDevice(id, name, deviceType, targetType, apiVersion ?? targetAPI, isPlayStore)
                    );
                }
            }
        }
