...
describe('Preview utils tests', () => {
    ...
    it('Generates correct websocket url', async () => {
        ...
        expect(
            PreviewUtils.generateWebSocketUrlForLocalDevServer('ios', { httpPort: 1111, httpsPort: 1112 })
        ).to.be.equal('wss://localhost:1112');

        // ensure that for Android the proper url is generated
        expect(
            PreviewUtils.generateWebSocketUrlForLocalDevServer('android', { httpPort: 1111, httpsPort: 1112 })
        ).to.be.equal('wss://10.0.2.2:1112');
        ...
        expect(
            PreviewUtils.generateWebSocketUrlForLocalDevServer('desktop', { httpPort: 1111, httpsPort: 1112 })
        ).to.be.equal('ws://localhost:1111');
        stub1.restore();
