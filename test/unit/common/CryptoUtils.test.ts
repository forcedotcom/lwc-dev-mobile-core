...
describe('CryptoUtils tests', () => {
    ...
    it('getDERCertSubjectHashOld succeeds to generate subject hash', async () => {
        const hash = CryptoUtils.getSubjectHashOld(Buffer.from(testCertData.derCertificate, 'binary'));
        expect(hash).to.be.equal(testCertData.expectedSubjectHash);
    });

    it('getPEMCertSubjectHashOld succeeds to generate subject hash', async () => {
        const hash = CryptoUtils.getSubjectHashOld(undefined, testCertData.pemCertificate);
        expect(hash).to.be.equal(testCertData.expectedSubjectHash);
    });
